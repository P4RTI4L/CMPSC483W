...
public class DetailModel {
	...
	// Synchronously queries the database given an id of a movie for the basic
	// information of a movie
	public static DetailData synchronousDetailPrimaryRetrieve(int movieId) {

		JSONObject result = TmdbModel.executeQuery("movie/" + movieId, null,
				null);
		...
		if (result != null) {
			try {
				...
				return new DetailData(result.getBoolean("adult"), genres,
						result.getInt("id"), result.getString("overview"),
						result.getDouble("popularity"),
						result.getString("poster_path"),
						result.getString("release_date"),
						result.getInt("runtime"), result.getString("title"),
	...
	// Synchronously queries the database given an id of a movie for a list of
	// the names of the actors
	public static ArrayList<String> synchronousDetailCastRetrieve(int movieId) {

		JSONObject query = TmdbModel.executeQuery(
				"movie/" + movieId + "/casts", null, null);
