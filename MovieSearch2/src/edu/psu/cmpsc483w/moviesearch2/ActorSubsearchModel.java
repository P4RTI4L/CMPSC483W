...
public class ActorSubsearchModel extends PagedModel {
	...
	// Overloaded version of synchronousActorSearch that allows one to specify
	// the page of results
	private static Pair<Object[], Integer> synchronousActorSubsearch(
			String nameSubstring, int page) {
		JSONObject json = TmdbModel.executeQuery("search/person", new String[] {
				"query", "page" },
		...
		try {
			...
			for (int i = 0; i < resultsArray.length(); i++) {
				...
				actors[i] = new ActorData(entry.getBoolean("adult"),
						entry.getString("name"), entry.getInt("id"),
	...
	protected Pair<Object[], Integer> fetchNewResults() {
		return synchronousActorSubsearch(this.name, this.nextPage);
	}

	@Override
	public long getDataId(int position) {
		ActorData data = (ActorData) this.getData(position);
