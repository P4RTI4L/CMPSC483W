...
public class TmdbModel {
	...
	/**
	 * Returns the image base url if initialized, otherwise queries tmdb for the
	 * url and returns that value
	 * 
	 * @return Base URL for images
	 */
	...
	/**
	 * Returns a complete image url given a type and a relative path (with a
	 * leading slash) and a suggested width for the image
	 * 
	 * @param type
	 *            this.PROFILE_IMAGE or this.POSTER_IMAGE, the type of image
	 *            being fetched.
	 * @param path
	 *            The relative path of the image URL to get
	 * @param width
	 *            The width of the space the image will be displayed in
	...
	/**
	 * Returns the first poster size larger than width, returns the last entry
	 * if none of the specified are large enough
	 * 
	 * @param width
	 *            The width of the space to find
	 * @return A string representation of the best size.
	 */
	private static String getBestPosterSize(int width) {
		...
		for (int i = 0; i < posterSizes.length; i++) {
			// All poster sizes are in the format wx (where x is the width) so
			// check for w first, if not return the last size
	...
	/**
	 * Returns the first profile size larger than width, returns the last entry
	 * if none of the specified are large enough
	 * 
	 * @param width
	 *            The width of the space to fit
	 * @return String representation of the best size
	 */
	private static String getBestProfileSize(int width) {
		...
		for (int i = 0; i < profileSizes.length; i++) {
			// All profile sizes are in the format wx (where x is the width) so
			// check for w first, if not return the last size
			// there may be a format starting with hy (for height) but just
			// avoid due to layout control
	...
	/**
	 * Initializes the poster sizes, base image URL, and profile sizes
	 */
	...
	/**
	 * Calls the TMDB api with the given request and returns a JSON object with
	 * the server's response
	 * 
	 * @param request
	 *            a string containing the type of request (following the "/3/"
	 *            of the url without needing a leading slash)
	 * @param parameters
	 *            a list of parameters to add to the query
	 * @param parameterValues
	 *            a list of values for the corresponding strings in parameters
	 * @return The results
	 */
	public static JSONObject executeQuery(String request, String parameters[],
		...
		if ((parameters != null) && (parameterValues != null)) {
			for (int i = 0; i < parameters.length; i++) {
				// Just check if in bounds in case the two arrays are not of
				// equal length
				if (i < parameterValues.length) {
					...
					try {
						url += "&"
								...
								+ "="
								+ URLEncoder
										.encode(parameterValues[i], "UTF-8");
	 ...
	 * -client-at-android/ Converts the InputStream to a String by using a
	 * BufferedReader to iterate until it returns null.
	 * 
	 * @param is
	 *            Stream to convert
