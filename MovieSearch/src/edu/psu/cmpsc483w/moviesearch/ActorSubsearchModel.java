...
public class ActorSubsearchModel {
	...
	// Synchronously queries the database given a substring of a name to search
	// for
	// the synchronous version is provided to make unit testing easier and can
	// be transformed
	// easily into the asynchronous version
	//
	public static Pair<ActorData[], Integer> synchronousActorSearch(
			String nameSubstring) {
		return synchronousActorSearch(nameSubstring, null);
	}

	// Overloaded version of synchronousActorSearch that allows for specifyingw
	// the page
	// Omits the page from the query if page is null
	public static Pair<ActorData[], Integer> synchronousActorSearch(
			String nameSubstring, String page) {
		try {
			String url = REQUEST_URL + "?api_key=" + API_KEY + "&query="
					+ URLEncoder.encode(nameSubstring, "UTF-8");
			if (page != null) {
				url += "&page=" + page;
			}
		...
		try {
			...
			if (entity != null) {
				...
				try {
					...
					resultPair = new Pair<ActorData[], Integer>(actors,
							numPages);
