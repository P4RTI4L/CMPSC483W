...
public class ResultsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		else
		{
			...
			{
				new AsyncTaskMovieQuery().execute(intent.getStringExtra("query"));
			}
		}
	...
	// Custom adapter for the movie listings ListView
		private class MovieResultsArrayAdapter extends ArrayAdapter<MovieListingData> {
			...
			public MovieResultsArrayAdapter(Activity context, ArrayList<MovieListingData> movies)
			{
			...
			public View getView(final int position, View convertView, ViewGroup parent)
			{
				...
				if (rowView == null)
				{
					rowView = context.getLayoutInflater().inflate(R.layout.results_row, null);
	...
	// Web queries should always be performed asynchronously to prevent blocking the UI thread, for this purpose
		...
		private class AsyncTaskMovieQuery extends AsyncTask<String, Integer, ArrayList<MovieListingData>>
		{
			@Override
